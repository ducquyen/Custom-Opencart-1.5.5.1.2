...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
				$.ajax({
					url: 'index.php?route=common/filesmanager/files&token=<?php echo $token; ?>&files=' + encodeURIComponent('data/' + $(element).find('input[name=\'files\']').attr('value')),
					dataType: 'html',
	...
	$('#column-left').tree({
		...
		data: { 
			...
			opts: { 
				method: 'post', 
				url: 'index.php?route=common/filesmanager/directory&token=<?php echo $token; ?>'
			} 
		},
		selected: 'top',
		ui: {		
		...
		callback: {
			...
			onselect: function (NODE, TREE_OBJ) {
				$.ajax({
					url: 'index.php?route=common/filesmanager/files&token=<?php echo $token; ?>',
					type: 'post',
					...
					success: function(json) {
						...
						if (json) {
							for (i = 0; i < json.length; i++) {
								html += '<a alt="" title=" '+ json[i]['filename'] + '" >' + ((json[i]['filename'].length > 15) ? (json[i]['filename'].substr(0, 15) + '..') : json[i]['filename']) + '<br />' + json[i]['size'] + '<input type="hidden" name="files" value="' + json[i]['file'] + '" /></a>';
							}
						}
	...
	$('#column-right a').live('dblclick', function() {
		<?php if ($fckeditor) { ?>
		window.opener.CKEDITOR.tools.callFunction(<?php echo $fckeditor; ?>, '<?php echo $directory; ?>' + $(this).find('input[name=\'files\']').attr('value'));
		
		self.close();	
		<?php } else { ?>
		parent.$('#<?php echo $field; ?>').attr('value', 'data/' + $(this).find('input[name=\'files\']').attr('value'));
	...
	$('#create').bind('click', function() {
		...
		if (tree.selected) {
			...
			$('#dialog input[type=\'button\']').bind('click', function() {
				$.ajax({
					url: 'index.php?route=common/filesmanager/create&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#delete').bind('click', function() {
		path = $('#column-right a.selected').find('input[name=\'files\']').attr('value');
							 
		if (path) {
			$.ajax({
				url: 'index.php?route=common/filesmanager/delete&token=<?php echo $token; ?>',
				type: 'post',
	...
	$('#move').bind('click', function() {
		...
		$('#dialog select[name=\'to\']').load('index.php?route=common/filesmanager/folders&token=<?php echo $token; ?>');
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=common/filesmanager/move&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#copy').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=common/filesmanager/copy&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#rename').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {		
				$.ajax({
					url: 'index.php?route=common/filesmanager/rename&token=<?php echo $token; ?>',
					type: 'post',
					data: 'path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent($('#dialog input[name=\'name\']').val()),
					dataType: 'json',
					success: function(json) {
	...
	new AjaxUpload('#upload', {
		action: 'index.php?route=common/filesmanager/upload&token=<?php echo $token; ?>',
		name: 'files',
		autoSubmit: false,
		responseType: 'json',
