<?php
class ControllerCommonFileManagerFix extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->template = 'common/filemanagerfix.tpl';
	...
	public function files() {
		...
		$allowed=array_merge($allowed, $allowed1);
		...
		if ($files) {
			foreach ($files as $file) {
				...
				if (in_array(strtolower($ext), $allowed)) {
					...
					if (in_array(strtolower($ext), $allowed1)){
					
						$thumb = $this->model_tool_image->resize(utf8_substr(DIR_IMAGE .str_replace('.','',$ext).'.jpg', strlen(DIR_IMAGE)), 100, 100);
					}else{
	...
	public function upload() {
		...
		$json = array();
			...
			if ( isset($this->request->get['CKEditorFuncNum']) ) {
				$this->request->post['directory'] = 'userfiles';
		...
		if (isset($this->request->post['directory'])) {	
			if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
				...
				print $this->request->files['image']['type'];		
		...
		if (!$this->user->hasPermission('modify', 'common/filemanagerfix')) {
      		$json['error'] = $this->language->get('error_permission');  
    	}
		
		if (!isset($json['error'])) {	
			if (@move_uploaded_file($this->request->files['upload']['tmp_name'], $directory . '/' . $filename)) {		
		...
		if ( isset($this->request->get['CKEditorFuncNum']) ) {
			if ( $json['success'] ) {
				echo "<script type='text/javascript'>window.parent.CKEDITOR.tools.callFunction(".$this->request->get['CKEditorFuncNum'].", '/image/data/".$this->request->post['directory']."/".$this->request->files['image']['name']."');</script>";
			} else {
				echo $json['error'];
			}
