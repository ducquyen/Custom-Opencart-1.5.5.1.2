<?php
class ControllerCommonFilesManager extends Controller {
	private $error = array();
	
	public function index() {
		$this->language->load('common/filesmanager');
		...
		$this->data['directory'] = HTTP_CATALOG . 'download/data/';
		...
		$this->template = 'common/filesmanager.tpl';
	...
	public function files() {
		...
		if ($files) {
			...
			foreach ($files as $file) {
				...
				if (!in_array(strtolower($ext), $notallowed)) {
					$size = filesize($file);
					...
					$json[] = array(
						...
						'file'     => utf8_substr($file, utf8_strlen(DIR_DOWNLOAD . 'data/')),
	...
	public function create() {
		...
		if (!$this->user->hasPermission('modify', 'common/filesmanager')) {
	...
	public function upload() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['files']) && $this->request->files['files']['tmp_name']) {
				$filename = basename(html_entity_decode($this->request->files['files']['name'], ENT_QUOTES, 'UTF-8'));
				...
				if ($this->request->files['files']['size'] > 300000000) {
				...
				if (in_array($this->request->files['files']['type'], $notallowed)) {
				...
				if (in_array(strtolower(strrchr($filename, '.')), $notallowed)) {
					$json['error'] = $this->language->get('error_file_type');
				}

				if ($this->request->files['files']['error'] != UPLOAD_ERR_OK) {
					$json['error'] = 'error_upload_' . $this->request->files['files']['error'];
				}			
			} else {
		...
		if (!isset($json['error'])) {	
			if (@move_uploaded_file($this->request->files['files']['tmp_name'], $directory . '/' . $filename)) {		
